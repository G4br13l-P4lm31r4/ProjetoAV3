 ...
 * Ponto de entrada da aplicação da Locadora de Carros.
 * Contém o menu principal e os submenus de cada entidade,
 * além de pré-cadastrar dados de exemplo ao inicializar o sistema.
 *
 * Estrutura de menus:
 *   1. Carros Econômicos  → incluir / alterar / excluir / listar / buscar
 *   2. Carros de Luxo     → idem
 *   3. Carros             → idem
 *   4. Motos              → idem
 *   5. Vans               → idem
 *   6. Clientes           → idem
 *   7. Funcionários       → idem
 *   8. Locações           → incluir / alterar / encerrar / cancelar / listar / buscar
 *   0. Sair
 */
public class Programa {

    // =========================================================================
    // Listas globais que armazenam todos os objetos durante a execução
    // =========================================================================
    ...
    // Scanner único reutilizado em todo o programa
    private static Scanner sc = new Scanner(System.in);

    // Contadores de IDs (auto-incremento simples)
     ...
     * Pré-cadastra os dados de exemplo e exibe o menu principal.
     * @param args argumentos de linha de comando (não utilizados)
     */
    public static void main(String[] args) {
        preCadastrar();   // Carrega dados de exemplo
        menuPrincipal();  // Entra no loop do menu
        sc.close();
        System.out.println("\nSistema encerrado. Até logo!");
    }

    // =========================================================================
    // PRÉ-CADASTRO — pelo menos 7 objetos de cada classe
    // =========================================================================

    /**
     * Pré-cadastra automaticamente objetos de cada classe ao iniciar o sistema.
     * Garante que os menus e buscas já tenham dados para demonstração.
     */
    private static void preCadastrar() {
        ...
        // ---- Locações (7) — usa veículos e pessoas já cadastrados ----
        // Nota: ao criar Locacao, o veículo é automaticamente marcado como locado
    ...
    /**
     * Exibe o menu principal e direciona para os submenus correspondentes.
     */
    ...
    // =========================================================================
    // MENU CARRO ECONÔMICO
    // =========================================================================

    /** Submenu de gerenciamento de Carros Econômicos. */
    ...
    /** Lê dados do console e inclui um novo CarroEconomico na lista. */
    private static void incluirCarroEconomico() {
        ...
        CarroEconomico ce = new CarroEconomico(
                proxIdVeiculo++, placa, marca, modelo, ano, km, portas, comb, pm, cons);
        carrosEconomicos.add(ce);
        System.out.println("Carro Econômico ID " + ce.getId() + " incluído com sucesso!");
    }

    /** Lê o ID e permite alterar os dados de um CarroEconomico existente. */
    ...
    /** Busca CarroEconomico pelo ID. @return o objeto ou null */
    private static CarroEconomico buscarCarroEconomicoPorId(int id) {
        for (CarroEconomico ce : carrosEconomicos)
    ...
    /** Submenu de gerenciamento de Carros de Luxo. */
    ...
    private static void incluirCarroLuxo() {
        ...
        CarroLuxo cl = new CarroLuxo(
                proxIdVeiculo++, placa, marca, modelo, ano, km, portas, comb, pm, itens, seg);
        carrosLuxo.add(cl);
        System.out.println("Carro de Luxo ID " + cl.getId() + " incluído com sucesso!");
    }

    /** Altera CarroLuxo existente. */
    ...
    /** Busca CarroLuxo pelo ID. @return o objeto ou null */
    ...
    /** Submenu de gerenciamento de Carros genéricos. */
    ...
    /** Busca Carro pelo ID. @return o objeto ou null */
    ...
    /** Submenu de gerenciamento de Motos. */
    ...
    /** Busca Moto pelo ID. @return o objeto ou null */
    ...
    /** Submenu de gerenciamento de Vans. */
    ...
    /** Busca Van pelo ID. @return o objeto ou null */
    ...
    /** Exclui Cliente da lista pelo ID. */
    ...
    /** Busca cliente por ID ou nome parcial. */
    ...
    /** Busca Cliente pelo ID. @return o objeto ou null */
    ...
    /** Exclui Funcionário da lista pelo ID. */
    ...
    /** Busca funcionário por ID ou nome parcial. */
    ...
    /** Busca Funcionário pelo ID. @return o objeto ou null */
    ...
    /** Submenu de gerenciamento de Locações. */
    ...
    /** Abre nova locação selecionando cliente, veículo e funcionário. */
    private static void incluirLocacao() {
        ...
        // Seleciona o veículo disponível
        ...
        // Seleciona o funcionário responsável
        listarFuncionarios();
        ...
        locacoes.add(loc);
        System.out.printf("Locação ID %d criada! Valor total: R$ %.2f%n",
                loc.getId(), loc.getValorTotal());
    }

    /**
     * Encontra um veículo disponível em qualquer lista pelo tipo e ID.
     * @param tipo 1=CarroEco, 2=CarroLuxo, 3=Carro, 4=Moto, 5=Van
     * @param id   ID do veículo
     * @return veículo disponível ou null
     */
    ...
    /** Altera observações de uma locação aberta. */
    private static void alterarLocacao() {
        ...
        if (!loc.getStatus().equals("ABERTA")) {
            System.out.println("Só é possível alterar locações com status ABERTA."); return;
        }
    ...
    /** Encerra uma locação aberta registrando a devolução real. */
    private static void encerrarLocacao() {
        ...
        if (loc == null) { System.out.println("Não encontrada."); return; }
        if (!loc.getStatus().equals("ABERTA")) {
            System.out.println("Locação já está " + loc.getStatus()); return;
        }
    ...
    /** Lista todas as locações com seu resumo. */
    ...
    /** Busca locação por ID e exibe descrição completa. */
    ...
    /** Busca Locacao pelo ID. @return o objeto ou null */
    private static Locacao buscarLocacaoPorId(int id) {
        for (Locacao l : locacoes) if (l.getId() == id) return l;
        return null;
    }

    /** Lista todos os veículos disponíveis para locação em todas as categorias. */
    private static void listarVeiculosDisponiveis() {
        System.out.println("\n=== VEÍCULOS DISPONÍVEIS ===");
        System.out.println("-- Carros Econômicos --");
        for (CarroEconomico ce : carrosEconomicos)
            if (ce.isDisponivel()) System.out.println(ce);

        System.out.println("-- Carros de Luxo --");
        for (CarroLuxo cl : carrosLuxo)
            if (cl.isDisponivel()) System.out.println(cl);

        System.out.println("-- Carros --");
        for (Carro c : carros)
            if (c.isDisponivel()) System.out.println(c);

        System.out.println("-- Motos --");
        for (Moto m : motos)
            if (m.isDisponivel()) System.out.println(m);
    ...
    // =========================================================================
    // MÉTODOS GENÉRICOS REUTILIZÁVEIS (usam polimorfismo via Veiculo)
    // =========================================================================

    /**
     * Exclui um veículo de qualquer lista pelo ID.
     * Veículos atualmente locados não podem ser excluídos.
     * @param lista  lista de veículos do tipo específico
     * @param titulo nome da categoria para exibição
     */
    private static <T extends Veiculo> void excluirVeiculo(List<T> lista, String titulo) {
        ...
        if (!alvo.isDisponivel()) {
            System.out.println("Veículo está locado e não pode ser excluído."); return;
        }
    ...
    /**
     * Lista todos os veículos de uma categoria com descrição simples.
     * @param lista  lista de veículos do tipo específico
     * @param titulo nome da categoria para exibição
     */
    ...
    /**
     * Busca um veículo de uma categoria por ID ou placa.
     * @param lista  lista de veículos do tipo específico
     * @param titulo nome da categoria para exibição
     */
    ...
    // =========================================================================
    // UTILITÁRIOS DE LEITURA SEGURA DO TECLADO
    // =========================================================================

    /**
     * Lê um número inteiro do console com tratamento de erro.
     * Se o valor digitado não for inteiro, retorna -1 e limpa o buffer.
    ...
    /**
     * Lê um número de ponto flutuante (double) do console com tratamento de erro.
     * Aceita tanto ponto quanto vírgula como separador decimal.
     * Se o valor digitado não for numérico, retorna 0.0.
     * @return double lido ou 0.0 em caso de erro
     */
